...
from transformers import PreTrainedModel, AutoModelForSequenceClassification#这是含有model head（下游任务为因果的）的模型处理文件
...
class FOSSVModel(PreTrainedModel):#这个类最终形成了一个新的使用了FOSSV的模型
    ...
    def __init__(self, config):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(
            ...
            num_labels=2)#第二个参数表示是否信任从远程加载的模型代码。
    ...
    def _set_submodule(self, target: str, module: torch.nn.Module):#这个自定义函数是用于查找替换一个模块
        ...
        setattr(mod, name, module)# setattr(object, name, value)object是要修改的对象，name是要修改的object的具体的属性或者说子模块，module是要修改的目标


    def forward(self, **kwargs):
        """
        Forward pass of the model. Ensures compatibility with LlamaForSequenceClassification.
    
        Only pass the expected arguments to the underlying model (input_ids, attention_mask, labels).
        """
        # Extract only the necessary arguments for LlamaForSequenceClassification
        # These typically include input_ids, attention_mask, and labels
        # 获取设备信息（模型所在设备）
        device = next(self.parameters()).device
        
        # 确保所有输入都被转移到相同的设备
        valid_args = {k: v.to(device) for k, v in kwargs.items() if k in ['input_ids', 'attention_mask', 'labels']}
    
        return self.model(**valid_args)
    ...
    def _replace_modules(self):
        ...
        for name, module in self.named_modules(): # 遍历模型的所有模块同时返回模块的名称和模块本身
            print(f"Processing module: {name}")  # 添加日志
            if any(t in name for t in target_module_names) and isinstance(
                ...
                print("Module replacement completed.")  # 添加日志
    ...
    def fossv_init(
        ...
        for module in self.modules():# 遍历所有模块
            if isinstance(module, FOSSVLinear): # 如果模块是FOSSVLinear类型
                print(f"Initializing FOSSVLinear: {module}")  # 添加日志
                module.fossv_init(weight_dtype, adapter_dtype)# 初始化FOSSV适配器
            print("FOSSV adapters initialization completed.")
